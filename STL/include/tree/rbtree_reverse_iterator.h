...
/*
* Custom reverse iterator for RBTree implementation by Seungtack Lee
...
namespace ST
{
	...
	public:
		...
		bool operator== (const rbtree_reverse_iterator& other) const { return (m_Node == other.m_Node) && (m_Tree == other.m_Tree); }
		bool operator!= (const rbtree_reverse_iterator& other) const { return !(*this == other); }
		...
		{
			rbtree_reverse_iterator temp = *this;
			++(*this);
	...
	public:
		...
		rbtree_reverse_iterator(RBTree<T, Comparator>* tree, RBNode<T>* node)
			: m_Tree(tree)
			, m_Node(node)
		{
		}
